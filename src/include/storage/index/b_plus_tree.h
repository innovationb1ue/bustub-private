...
public:
  ...
  auto FindLeaf(const KeyType &key, Operation operation,
...
private:
  ...
  auto InsertIntoLeaf(const KeyType &key, const ValueType &value,
                      Transaction *transaction = nullptr) -> bool;
  ...
  template <typename N> auto Split(N *node) -> N *;

  template <typename N>
  auto CoalesceOrRedistribute(N *node, Transaction *transaction = nullptr)
      -> bool;
  ...
  template <typename N>
  void
  Redistribute(N *neighbor_node, N *node,
               BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *parent,
