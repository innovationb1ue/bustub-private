...
template <typename T> class TrieNodeWithValue;
...
public:
  ...
  // insert a node with value directly.
  template <typename T>
  std::unique_ptr<TrieNode> *
  InsertChildNode(char key_char,
                  std::unique_ptr<TrieNodeWithValue<T>> &&child) {
    if (HasChild(key_char) || key_char != child->key_char_) {
      return nullptr;
    }
    // forward the child to let it be rvalue.
    children_[key_char] =
        std::forward<std::unique_ptr<TrieNodeWithValue<T>>>(child);
    return &children_[key_char];
  }
  /**
   * TODO(P0): Add implementation
...
public:
  ...
  template <typename T> bool Insert(const std::string &key, T value) {
    ...
    while (c != key.end()) {
      ...
      // if previous node does not have a child with current char, we create
      // this path.
      if (!pre_child->get()->HasChild(*cur)) {
        pre_child = pre_child->get()->InsertChildNode(
            *cur, std::make_unique<TrieNode>(*cur));
      } else {
        // if this path is found, we move the pre_child pointer to next node.
        pre_child = pre_child->get()->GetChildNode(*cur);
      }
    }
    // after the while loop, pre_child point to the end node of the trie with
    // our key.
    c--;

    auto end_node = pre_child->get()->GetChildNode(*c);
    // if the end node already got a value in there. return false
    ...
    // end node is nullptr. So we need to insert a new node with this value.
    pre_child->get()->InsertChildNode(
        *c, std::make_unique<TrieNodeWithValue<T>>(*c, value));
    //    pre_child = pre_child->get()->InsertChildNode(*c,
    //    std::make_unique<TrieNode>(*c)); auto new_node2 = new
    //    TrieNodeWithValue(*c, value); pre_child->reset(new_node2);
  ...
  bool Remove(const std::string &key) {
    ...
    while (!s.empty()) {
      auto t = s.top();
      s.pop();
      auto mykey = std::get<0>(t);
      auto node = std::get<1>(t);
      auto flag = (*node)->GetChildNode(mykey);
      if (flag != nullptr && (*flag)->HasChildren()) {
        continue;
      }
      (*node)->RemoveChildNode(mykey);
    }
    latch_.WUnlock();
    return true;
  }
