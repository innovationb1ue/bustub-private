...
TableHeap::TableHeap(BufferPoolManager *buffer_pool_manager,
  ...
  auto first_page = reinterpret_cast<TablePage *>(
...
auto TableHeap::InsertTuple(const Tuple &tuple, RID *rid, Transaction *txn)
    -> bool {
  ...
  while (!cur_page->InsertTuple(tuple, rid, txn, lock_manager_, log_manager_)) {
    ...
    if (next_page_id != INVALID_PAGE_ID) {
      ...
      auto new_page = static_cast<TablePage *>(
      ...
      new_page->Init(next_page_id, BUSTUB_PAGE_SIZE, cur_page->GetTablePageId(),
                     log_manager_, txn);
      cur_page->WUnlatch();
...
auto TableHeap::UpdateTuple(const Tuple &tuple, const RID &rid,
...
auto TableHeap::GetTuple(const RID &rid, Tuple *tuple, Transaction *txn,
                         bool acquire_read_lock) -> bool {
  // Find the page which contains the tuple.
...
auto TableHeap::Begin(Transaction *txn) -> TableIterator {
  ...
  while (page_id != INVALID_PAGE_ID) {
    auto page =
        static_cast<TablePage *>(buffer_pool_manager_->FetchPage(page_id));
    page->RLatch();
...
auto TableHeap::End() -> TableIterator {
  return {this, RID(INVALID_PAGE_ID, 0), nullptr};
}
